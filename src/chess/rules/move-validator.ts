...
import { PieceMoveValidator } from "../types/piece-move-validator";
...
export class MoveValidator {
    ...
    private static validators: Record<PieceType, PieceMoveValidator> = {
        pawn: this.validatePawnMove,
        knight: this.validateKnightMove,
        bishop: this.validateBishopMove,
        rook: this.validateRookMove,
        queen: this.validateQueenMove,
        king: this.validateKingMove,
    };
    ...
    private static validateCastleMove(game: Game, move: Move): boolean {
        ...
        if (!this.isPathSafe(game, { from: move.from, to: move.to, piece: PieceType.King, color: move.color })) return false;
